...
public class ArtistActivity extends BaseActivity implements View.OnClickListener {
  ...
  private void loadBackDrop() throws UnsupportedEncodingException {
    ...
    if (mArtist.getmArtistName().equalsIgnoreCase(this.getString(R.string.no_artist))) return;

    String artistName = URLEncoder.encode(mArtist.getmArtistName(), "UTF-8");
    LogUtils.v(TAG, artistName);
    String urlFormat = String.format("%s?method=%s&lang=%s&artist=%s&format=json&api_key=%s", url,
        "artist.getInfo", "zh", artistName, API_KEY);
    ConnectivityManager network =
        (ConnectivityManager) this.getSystemService(Context.CONNECTIVITY_SERVICE);
    if (network.getActiveNetworkInfo() != null &&
        network.getActiveNetworkInfo().isAvailable() && !network.getActiveNetworkInfo()
        .isRoaming()) {
      RequestQueue queue = Volley.newRequestQueue(this);
      GsonRequest<FmArtist> request =
          new GsonRequest<>(urlFormat, FmArtist.class, new Response.Listener<FmArtist>() {
            @Override public void onResponse(FmArtist response) {
